...
class BaseController:
    ...
    def update_record(self, record_id):
        if not validate_uuid4(record_id):
            return jsonify({"message": "invalid record id"}), 400
        post_data = request.json
        get_by_id_query = f"""SELECT {",".join(self.post_data_fields)} FROM "{self.table_name}"
        WHERE {self.primary_key} = %s"""
        cursor.execute(get_by_id_query, (record_id,))
        record = cursor.fetchone()
        if not record:
        ...
        update_fields = ",".join(f"{field} = %s" for field in self.post_data_fields)
        ...
        try:
            cursor.execute(update_query, (*[post_data.get(field, value) for (field, value) in zip(self.post_data_fields, record)], record_id))
            record = cursor.fetchone()
            connection.commit()
        except:
            connection.rollback()
